...
fn main() -> io::Result<()> {
    ...
    let destinations: Vec<&str> = matches
        .value_of("destinations")
        .unwrap()
        .split(',')
        .collect();
    let upstreams: Arc<Mutex<Vec<Upstream>>> = Arc::new(Mutex::new(
        destinations.iter().map(|d| Upstream::new(d)).collect(),
    ));
    ...
    thread::spawn(move || loop {
        ...
        for upstream in upstreams.iter_mut() {
            if !upstream.is_live {
                match TcpStream::connect(format!("{}:{}", upstream.ip, upstream.port)) {
                    Ok(_) => {
    ...
    for stream in listener.incoming() {
        ...
        let mut dest = upstreams[dest_index % destinations.len()].borrow_mut();
        dest_index += 1;
        let mut iterations = 0;

        while !dest.is_live {
            dest_index += 1;
            dest = upstreams[dest_index % destinations.len()].borrow_mut();
            iterations += 1;
